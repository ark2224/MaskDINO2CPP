...
public:
    ...
    MSDeformAttnTransformerEncoderImpl(torch::nn::AnyModule& encoder_layer,
                                       int const& num_l) : 
    num_layers(num_l), torch::nn::Module() { layers = get_clones(encoder_layer, num_layers); }
    ...
    torch::Device device)
{
    ...
    for (auto const& shape : spatial_shapes) {
        ...
        std::vector<torch::Tensor> ref_xy = torch::meshgrid({
            ...
            torch::linspace(0.5, W_ - 0.5, W_, t
                            orch::TensorOptions().dtype(torch::kFloat32).device(device))
        });
...
torch::Tensor MSDeformAttnTransformerEncoderImpl::forward(
    ...
    torch::Tensor reference_points = get_reference_points(spatial_shapes, valid_ratios, src.device());
    output = layers->forward(output, pos, reference_points, spatial_shapes, level_start_index, padding_mask);
    return output;
}
...
torch::Tensor MSDeformAttnFunction::forward(torch::Tensor& value,
                                            ...
                                            int const& im2col_step)
{
    torch::Tensor output = ms_deform_attn_cuda_forward( //https://github.com/fundamentalvision/Deformable-DETR/blob/main/models/ops/src/cuda/ms_deform_attn_cuda.h
        value,
    ...
    save_for_backward(value, input_spatial_shapes, input_level_start_index, sampling_locations, attention_weights_tensor);
    return output;
}
...
std::vector<int> MSDeformAttnFunction::backward(torch::Tensor& grad_output) {
    ...
    int grad_value, grad_sampling_loc, grad_attn_weight = ms_deform_attn_cuda_backward(
...
torch::Tensor ms_deform_attn_core_pytorch(torch::Tensor& value, torch::Tensor& value_spatial_shapes,
...
public:
    ...
    MSDeformAttnImpl(int const&, int const&, int const&, int const&);
...
MSDeformAttnImpl::MSDeformAttnImpl(int const& d = 256, int const& n_levels = 4, int const& n_heads = 8, int const& n_points = 4) :
...
torch::Tensor MSDeformAttnImpl::forward(torch::Tensor& query, torch::Tensor& reference_pts, torch::Tensor& input_flatten,
...
public:
    MSDeformAttnTransformerEncoderLayer(int const&, int const&,
                                        float const&, std::string const&,
                                        int const&, int const&, int const&);
    static torch::Tensor with_pos_embed(torch::Tensor& tensor, torch::Tensor& pos)
    { 
        if (pos.defined())
            return tensor + pos;
        else
    ...
    torch::Tensor forward(torch::Tensor&, torch::Tensor&, torch::Tensor&, torch::Tensor&,
...
private:
    ...
    torch::nn::Sequential activation_layer = nullptr;
};

MSDeformAttnTransformerEncoderLayer::MSDeformAttnTransformerEncoderLayer(int const& d_model = 256,
                                                                        int const& d_ffn = 1024,
                                                                        float const& dropout = 0.1,
                                                                        std::string const& activation = "relu",
                                                                        int const& n_levels = 4,
                                                                        int const& n_heads = 8,
                                                                        int const& n_points = 4) : torch::nn::Module()
{
...
torch::Tensor MSDeformAttnTransformerEncoderLayer::forward(torch::Tensor& src, torch::Tensor& pos, torch::Tensor& reference_pts, torch::Tensor& input_flatten,
                          std::vector<torch::Tensor>& input_spatial_shapes, torch::Tensor& input_level_start_index,
                          std::initializer_list<torch::Tensor>& input_padding_mask_list) {
    torch::Tensor Spatial_shapes = torch::from_blob(input_spatial_shapes.data(),
...
public:
    MSDeformAttnTransformerEncoderOnly(int d = 256, int nhead = 8, const int& num_encoder_layers = 6,
    ...
    std::vector<torch::Tensor> forward(std::vector<torch::Tensor>&, std::vector<torch::Tensor>&, std::vector<torch::Tensor>&);
...
std::vector<torch::Tensor> MSDeformAttnTransformerEncoderOnly::forward(std::vector<torch::Tensor>& srcs,
...
// class MSDeformAttnFunction : torch::autograd::Function<torch::Tensor> {
// public:
//     static torch::Tensor forward(torch::Tensor&, torch::Tensor&, torch::Tensor&,
//                                  torch::Tensor&, torch::Tensor&, int const&);
//     static std::vector<int> backward(torch::Tensor&);
// };
...
// //needs work
// torch::nn::Sequential get_clones(torch::nn::AnyModule& module, int const& num_layers, bool const& layer_share = false) {
//     torch::nn::Sequential ret = torch::nn::Sequential();
//     torch::nn::ModuleDict RET = torch::nn::ModuleDict();
...
public:
    PositionEmbeddingSine(int const&, int const&, bool const&, double const&);
    torch::Tensor forward(torch::Tensor&, torch::Tensor);
    // skipped __repr__() method
...
PositionEmbeddingSine::PositionEmbeddingSine(int const& num_pos_feats = 64, int const& temperature = 10000,
                                             bool const& normalize = false, double const& scale = 0.) : 
                                             torch::nn::Module(), num_pos_feats(num_pos_feats),
...
torch::Tensor PositionEmbeddingSine::forward(torch::Tensor& x, torch::Tensor mask = torch::Tensor()) {
    if (!mask.defined())
        mask = torch::zeros({x.sizes()[0], x.sizes()[2], x.sizes()[3]}, torch::TensorOptions().device(x.device()).dtype(torch::kBool));
    ...
    torch::Tensor dimension_t = torch::arange(this->num_pos_feats, torch::TensorOptions().dtype(torch::kFloat32).device(x.device()));
    dimension_t = pow(this->temperature, (2 * torch::floor_divide(dimension_t, 2) / this->num_pos_feats));
...
public:
    MaskDINO2Encoder(std::set< std::pair<std::string, ShapeSpec> >&, float const&,
                     int const&, int const&, int const&, int const&, int const&, std::string const&,
                     std::vector<std::string>&, int const&, int const&, int const&, std::string const&);

    //took out from_config method
    // MaskDINO2Encoder from_config(CfgNode cfg);

    std::vector<torch::Tensor> forward_features(std::unordered_map<std::string, torch::Tensor>&, std::vector<torch::Tensor>&);

private:
    std::vector<std::string> in_features, transformer_in_features;
    std::vector<int> feature_strides, feature_channels, transformer_in_channels, transformer_feature_strides;
    std::string feature_order;
    int maskdino_num_feature_levels, common_stride, transformer_num_feature_levels, low_resolution_index, high_resolution_index, mask_dim, num_fpn_levels, total_num_feature_levels;
    MSDeformAttnTransformerEncoderOnly transformer;
    PositionEmbeddingSine pe_layer;
    torch::nn::Conv2d mask_features = nullptr;
    torch::nn::ModuleList input_proj = nullptr, lateral_convs = nullptr, output_convs = nullptr;

};

MaskDINO2Encoder::MaskDINO2Encoder(
    std::set< std::pair<std::string, ShapeSpec> >& input_shape, float const& transformer_dropout,
    int const& transformer_nheads, int const& transformer_dim_feedforward, int const& transformer_enc_layers,
    int const& conv_dim, int const& mask_dim, std::string const& norm,
    std::vector<std::string>& transformer_in_features_local, int const& common_stride,
    int const& num_feature_levels, int const& total_num_feature_levels, std::string const& feature_order = "low2high"
    ) : torch::nn::Module(), feature_order(feature_order), 
        total_num_feature_levels(total_num_feature_levels), common_stride(common_stride)
{
    ...
    if (feature_order == "low2high") {
        struct {
            bool operator()(std::pair<std::string, ShapeSpec>a, std::pair<std::string, ShapeSpec>b) const { return a.second.stride < b.second.stride; }            
        } strideLess;
    ...
    this->low_resolution_index = std::distance(transformer_in_channels.begin(),
                                               std::max_element(transformer_in_channels.begin(), transformer_in_channels.end()));
...
std::vector<torch::Tensor> MaskDINO2Encoder::forward_features(std::unordered_map<std::string, torch::Tensor>& features, std::vector<torch::Tensor>& masks) {
